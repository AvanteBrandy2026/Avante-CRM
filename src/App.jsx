...
const USERS = {
  ...
  // Partner accounts — Dashboard, B2B Customs and OKR access only
  'matt@117marketing.com':           { password: 'MattA1234',   rep: 'Matt A',  role: 'partner', name: 'Matthew Askew' },
  'john@newagency.co.za':            { password: 'John1234',    rep: 'John S',  role: 'partner', name: 'John Stanley' },
};
...
const isPartner = (u) => u?.role === 'partner';
...
// All people who can own OKR items or priorities (includes managers + partners)
const ALL_PEOPLE = [...SALES_REPS, 'Rob', 'Matt A', 'John S'];
...
function OKRPage({ currentUser, userIsManager }) {
  ...
  // ── CRUD ─────────────────────────────────────────────────────────────────
  const updateObjective = async (updated) => {
    const newList = objectives.map(o => o.id === updated.id ? updated : o);
    setObjectives(newList);
    await persistObjective(updated, newList);
  };
  ...
  const addObjective = async () => {
    ...
    const newObj = { id: `obj-${Date.now()}`, title: newObjTitle.trim(), quarter: OKR_QUARTER, collapsed: false, keyResults: [], assignees: [] };
    const newList = [...objectives, newObj];
    setObjectives(newList);
    await persistObjective(newObj, newList);
    setNewObjTitle(''); setAddingObj(false);
  };

  const updatePriority = async (updated) => {
    const newList = priorities.map(p => p.id === updated.id ? updated : p);
    setPriorities(newList);
    await persistPriority(updated, newList);
  };
  ...
  const addPriority = async () => {
    ...
    const newList = [...priorities, newPri];
    setPriorities(newList);
    await persistPriority(newPri, newList);
      ...
      <div style={{ flex:1, minWidth:0 }}>
        {loading ? (
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:200, gap:10 }}>
            <div style={{ width:20, height:20, border:'3px solid rgba(0,40,85,0.1)', borderTop:'3px solid #002855', borderRadius:'50%', animation:'spin 0.8s linear infinite' }} />
